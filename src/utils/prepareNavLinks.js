export const prepareNavLinks = (pages) => {
  ...
  subpages.forEach((page) => {
    ...
    const showInNavigation = !(type !== 'event_landing' && page.elements.show_in_navigation.value[0].name.toLowerCase() === "no")
    const pageSlug = page.elements.url
      ? page.elements.url.value.replace(/^\/+|\/+$/g, '').toLowerCase().trim()
      : page.elements.seo__slug?.value.replace(/^\/+|\/+$/g, '').toLowerCase().trim()
