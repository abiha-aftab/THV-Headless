    ...
    <div className='card'>
      {
        media && media.map((item, index) => {
          ...
          switch (type) {
            case 'video':
              const { video_id: { value: video_id }, thumbnail: {value: thumbnail} } = elements;
              return <div className="card__video" key={index}>
                ...
                <div className="card__video-footer">
                  ...
                  <div className='card__video-footer-watch-now'>
                    <a href={`https://youtu.be/${video_id}`} target="_blank"> Watch now </a>
                    <FaChevronRight />
                  </div>
                </div>
              </div>
            case 'image':
              const { image: { value: [image] }, image_alt: { value: alt } } = elements;
