...
const FooterDefault = ({
  ...
  const { id, codename } = footer.system
    ...
    <div className="footerDefault section">
      ...
      <div className="container">
        <div className="footerDefault__navigations">
          {social_sharing__text && <p>{social_sharing__text.value}</p>}
                ...
                {social_sharing__sources.value.map((source, key) => {
                  return (
                    <button
                      key={key}
                      onClick={() =>
                        window.open(`${source.name}${path}`, '_blank', 'share')
                      }
                      rel="noreferrer"
                      className="icon"
                    >
                      <span className="d-none">{source.codename}</span>
                      {source.codename === 'facebook' && (
                        <RiFacebookCircleFill />
                      )}
